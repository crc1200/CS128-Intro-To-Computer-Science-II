...
void Bank::add_surplus(double amount) {
  std::transform(accounts_.begin(), accounts_.end(), accounts_.begin(), add_money(amount));
}
...
std::vector<Account*> Bank::accounts_in_debt() {
  ...
  std::copy_if(accounts_.begin(), accounts_.end(), std::back_inserter(debtors), in_debt);
  return debtors;
}
...
std::vector<Account*> Bank::rich_accounts(double threshold) {
  ...
  std::copy_if(accounts_.begin(), accounts_.end(), back_inserter(to_eat), is_rich(threshold));
  return to_eat;
}
...
double Bank::average_balance() {
  return std::accumulate(accounts_.begin(), accounts_.end(), 0, accumulate_sum) / static_cast<double>(accounts_.size());
}
...
std::function<bool(Account*)> Bank::in_debt = [] (Account* a) {
  return a->balance < 0;
};
// NOTE: This is a function that returns a function
...
std::function<double(double, Account*)> Bank::accumulate_sum = [] (double amount, Account* a) {
